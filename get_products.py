...
#json : utilisé pour manipuler des données JSON.
#time : utilisé pour insérer des pauses dans l'exécution du programme.
...
#cookies : informations de session et de suivi du site web. Cela permet de simuler une requête provenant d'un navigateur et d'éviter d'être bloqué par des mesures de sécurité.
cookies = {
...
#headers : informations supplémentaires sur la requête, comme le type de contenu accepté (application/json) et l'agent utilisateur (user-agent). Cela permet de simuler une requête d'un navigateur spécifique.
headers = {
...
# while True : boucle infinie qui continue tant qu'il y a des données à récupérer.


page = 1
while True:
    #url : construit l'URL de la requête en ajoutant le paramètre currentPage pour récupérer les pages successives.
    url = f"{base_url}&currentPage={page}"
    #requests.get() : envoie une requête GET à l'API.
    response = requests.get(url, cookies=cookies, headers=headers)

    # Vérifier le code de réponse en 
    if response.status_code != 200:
        print(f"Erreur lors de la récupération de la page {page}. Code de statut:", response.status_code)
        #break : arrête la boucle si la requête échoue.
        break

    # response.json() : convertit la réponse de l'API en un dictionnaire Python.
